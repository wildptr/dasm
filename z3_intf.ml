...
module Make (V : VarType) = struct
  ...
  let rec of_sexp lb sexp =
    ...
    | Atom s ->
      ...
      else if String.left s 1 = "|" && String.right s 1 = "|" then
        let len = String.length s in
        E_var (String.sub s 1 (len-2) |> V.of_string)
      else lb s
    | List l ->
      begin match l with
        | Atom s :: tl ->
