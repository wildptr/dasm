...
let rec of_z3expr_app (expr : E.expr) =
  let f = E.get_func_decl expr in
  match Z3.FuncDecl.get_decl_kind f with
  | OP_UNINTERPRETED ->
    let name = Z3.Symbol.get_string (Z3.FuncDecl.get_name f) in
    parse_symbol name
  | OP_BNOT ->
    let e1 = get_1arg expr in
    E_prim1 (P1_lognot, e1)
  | OP_BAND ->
    let e1, e2 = get_2args expr in
    E_prim2 (P2_logand, e1, e2)
  | OP_BXOR ->
    let e1, e2 = get_2args expr in
    E_prim2 (P2_logxor, e1, e2)
  | OP_BOR ->
    let e1, e2 = get_2args expr in
    E_prim2 (P2_logor, e1, e2)
  | OP_CONCAT ->
    mk_prim2 P2_concat (get_args expr)
  | OP_AND ->
    mk_prim2 P2_and (get_args expr)
  | OP_OR ->
    mk_prim2 P2_or (get_args expr)
  (*| OP_EXTRACT ->
    _*)
  | _ -> failwith ("unimplemented: " ^ Z3.FuncDecl.to_string f)

and get_1arg (expr : E.expr) =
  let [e] = E.get_args expr in
  of_z3expr e

and get_2args (expr : E.expr) =
  let [e1;e2] = E.get_args expr in
  of_z3expr e1, of_z3expr e2

and get_args (expr : E.expr) =
  E.get_args expr |> List.map of_z3expr

and of_z3expr (expr : E.expr) =
  let ast = E.ast_of_expr expr in
  match Z3.AST.get_ast_kind ast with
  | NUMERAL_AST -> failwith "NUMERAL"
  | APP_AST -> of_z3expr_app expr
  | VAR_AST -> failwith "VAR"
  | QUANTIFIER_AST -> failwith "QUANTIFIER"
  | SORT_AST -> failwith "SORT"
  | FUNC_DECL_AST -> failwith "FUNC_DECL"
  | UNKNOWN_AST -> failwith "UNKNOWN"
