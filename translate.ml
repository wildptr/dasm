...
let wrong_arity name =
  raise (Error (name ^ ": wrong number of arguments"))

let unbound_symbol name =
  raise (Error ("unbound symbol: " ^ name))

let unknown_primitive name =
  raise (Error ("unknown primitive: " ^ name))
...
let extend_symtab (name, value) st =
  ...
  then raise (Error ("redefinition: " ^ name))
    ...
    begin match loc with
      | Loc_Var name ->
        begin match try_lookup st name with
          ...
          | _ -> raise (Error ("invalid assignment to " ^ name))
        end
      | Loc_Mem memloc ->
...
let translate_proc st proc =
  if debug then
    Format.printf "translating procedure %s@." proc.p_name;
  (* construct static environment to translate function body in *)
  let args =
    proc.p_params |> List.map begin fun (name, te) ->
      let t = translate_typ st te in name, t
    end
  in
  let rets =
    proc.p_results |> List.map begin fun (name, te) ->
      let t = translate_typ st te in name, t
    end
  in
  let ast_locals, stmts = proc.p_body in
  let locals =
...
let translate_ast ast =
  ast |> List.fold_left begin fun st decl ->
    ...
    | D_Proc proc ->
      ...
      extend_symtab (proc.p_name, Proc proc') st
    | D_Proc_Templ pt ->
    ...
    | D_Proc_Inst pi ->
      ...
      in
      let proc =
        { p_name = pi.pi_inst_name;
          p_params = pt.pt_proc_params;
          p_body = pt.pt_body;
          p_results = pt.pt_results }
      in
